...
class OdontoApp {
  ...
  renderCash() {
    ...
    document.getElementById('cash-payments').innerHTML = report.payments.length ? report.payments.map(item => `<article class="glass-card p-4 rounded-2xl flex justify-between gap-3"><div><strong>${escape(item.patientName)}</strong><small class="block text-slate-500">${item.voucherNumber} · ${item.invoiceNumber} · ${new Date(item.paidAt).toLocaleString('es-ES')}</small><small class="block">${item.lines.map(line => `${methodLabels[line.method] || escape(line.method)}${line.cardBrand ? ' ' + escape(line.cardBrand) + ' ' + escape(line.cardType) : ''}: ${money(line.amount)}`).join(' · ')}</small><small class="block">Seguro ${item.coveragePercent}%: ${money(item.insuranceCovered)} · Debía pagar: ${money(item.patientPaid)}</small><small class="block font-bold">Entregó: ${money(item.amountReceived)} · Devuelta: ${money(item.change)}</small><small class="block">Cobró: ${escape(item.receivedByName)}${item.reference ? ` · Ref: ${escape(item.reference)}` : ''}</small><button type="button" class="cash-reprint secondary-button mt-2" data-id="${item.id}">Reimprimir voucher</button></div><b class="text-emerald-700">${money(item.patientPaid)}</b></article>`).join('') : '<p class="text-sm text-slate-500 p-4">No hay cobros en este período.</p>';
    document.querySelectorAll('.cash-reprint').forEach(button => button.addEventListener('click', () => { const payment = report.payments.find(item => item.id === Number(button.dataset.id)); this.printCashVoucher(payment, null, payment.patientName); }));
  }
  ...
  printCashVoucher(payment, card, patientName = '', existingWindow = null) {
    ...
    const patient = patientName || card.querySelector('strong').textContent; const money = value => `RD$${Number(value).toFixed(2)}`;
    const lines = (payment.payments || payment.lines || []).map(line => `<div>${line.method}${line.cardBrand ? ' ' + line.cardBrand + ' ' + line.cardType : ''}<span>${money(line.amount)}</span></div>`).join('');
    receipt.document.open();
    receipt.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${payment.voucherNumber}</title><style>@page{size:80mm auto;margin:4mm}body{width:72mm;font:12px monospace;color:#111}h1,h2{text-align:center}hr{border:0;border-top:1px dashed}.row,div{display:flex;justify-content:space-between;margin:5px 0}.center{text-align:center;display:block}</style></head><body><h2>${this.config.nombreClinica}</h2><h1>COMPROBANTE DE PAGO</h1><hr><p>Comprobante: ${payment.voucherNumber}<br>Factura: FAC-${String(payment.consultationId).padStart(8,'0')}<br>Fecha: ${new Date(payment.paidAt).toLocaleString('es-DO')}<br>Paciente: ${patient}<br>Cajero: ${payment.receivedByName}<br>Usuario: ${payment.cashierUsername}</p><hr>${lines}<hr><div><b>TOTAL PAGADO</b><b>${money(payment.patientPaid)}</b></div><div>Entregado<span>${money(payment.amountReceived)}</span></div><div>Cambio<span>${money(payment.change)}</span></div><hr><p class="center">GRACIAS POR SU PAGO</p><script>onload=()=>setTimeout(()=>print(),250)<\/script></body></html>`); receipt.document.close();
  }
