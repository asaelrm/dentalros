...
class OdontoApp {
  ...
  applyPermissions() {
    ...
    const dashboardButton=document.getElementById('btn-dashboard'); if(dashboardButton) dashboardButton.hidden=!this.isAdmin();
  ...
  renderClinicalConsents(){const list=document.getElementById('clinical-consents-list');const patient=document.getElementById('consent-patient-signature');const professional=document.getElementById('consent-professional-signature');if(!list)return;const escape=window.escapeHTML;const options=(type)=>this.currentSignatures.filter(item=>type==='patient'?['paciente','responsable'].includes(item.signerType):item.signerType==='personal').map(item=>`<option value="${item.id}">${escape(item.signerName)}</option>`).join('');patient.innerHTML='<option value="">Firma del paciente (opcional)</option>'+options('patient');professional.innerHTML='<option value="">Firma profesional (opcional)</option>'+options('professional');list.innerHTML=(this.currentConsents||[]).map(item=>`<article class="p-3 border rounded-xl"><b>${escape(item.procedureName)}</b><small class="block">${new Date(item.createdAt).toLocaleString('es-DO')} · ${escape(item.createdByName)}</small><p class="text-xs mt-1 whitespace-pre-wrap">${escape(item.content)}</p><button class="btn-print-consent secondary-button mt-2" data-id="${item.id}">Ver / imprimir PDF</button></article>`).join('')||'<p class="text-xs text-slate-500">No hay consentimientos guardados.</p>';list.querySelectorAll('.btn-print-consent').forEach(button=>button.addEventListener('click',()=>this.printConsent(Number(button.dataset.id))));document.getElementById('btn-save-consent').onclick=async()=>{try{const procedureName=document.getElementById('consent-procedure').value.trim();const content=document.getElementById('consent-content').value.trim();await window.odontoDB.saveConsentimiento(this.currentPacienteId,{procedureName,content,patientSignatureId:patient.value||null,professionalSignatureId:professional.value||null});this.currentConsents=await window.odontoDB.getConsentimientos(this.currentPacienteId);this.renderClinicalConsents();document.getElementById('consent-procedure').value='';document.getElementById('consent-content').value='';this.showToast('Consentimiento guardado en la Historia Clínica.');}catch(error){this.showToast(error.message,'error');}};}
  printConsent(id){const item=(this.currentConsents||[]).find(row=>row.id===id);if(!item)return;const escape=window.escapeHTML;const patient=this.currentPaciente;const signatures=this.currentSignatures||[];const findSignature=signatureId=>signatures.find(row=>row.id===signatureId);const patientSignature=findSignature(item.patientSignatureId);const professionalSignature=findSignature(item.professionalSignatureId);const win=window.open('','_blank','width=850,height=900');if(!win)return this.showToast('Permite ventanas emergentes para ver el consentimiento.','error');const signature=entry=>entry?`<img src="${entry.imageUrl}" style="height:55px;max-width:220px;object-fit:contain"><br><b>${escape(entry.signerName)}</b>`:'<br><br>____________________________';win.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Consentimiento ${escape(item.procedureName)}</title><style>@page{size:letter;margin:15mm}body{font-family:Segoe UI,Arial;color:#334155;line-height:1.55}header{border-bottom:3px solid #9f1239;padding-bottom:12px;display:flex;gap:12px;align-items:center}header img{width:64px;height:64px;object-fit:contain}h1{color:#881337;margin:0}h2{color:#9f1239;margin-top:28px}.box{background:#fff1f2;border:1px solid #fecdd3;border-radius:10px;padding:12px}.signatures{display:flex;justify-content:space-between;gap:30px;margin-top:70px;text-align:center}.signatures>div{width:45%;border-top:1px solid #64748b;padding-top:6px}@media print{button{display:none}}</style></head><body><header><img src="${window.LOGO_DATA_URL||'img/logo.jpg'}"><div><h1>${escape(this.config.nombreClinica||'DentalRos')}</h1><small>${escape(this.config.direccion||'')} · ${escape(this.config.telefono||'')}</small></div></header><h2>CONSENTIMIENTO INFORMADO</h2><div class="box"><b>Paciente:</b> ${escape(`${patient.nombre} ${patient.apellido}`)}<br><b>Procedimiento:</b> ${escape(item.procedureName)}<br><b>Fecha:</b> ${new Date(item.createdAt).toLocaleString('es-DO')}</div><p style="white-space:pre-wrap">${escape(item.content)}</p><div class="signatures"><div>${signature(patientSignature)}<br><small>Paciente o responsable</small></div><div>${signature(professionalSignature)}<br><small>Profesional responsable</small></div></div><script>onload=()=>setTimeout(()=>print(),300)<\/script></body></html>`);win.document.close();}
  ...
  renderFacturas() {
    ...
    if (estimates) {
      const escape = window.escapeHTML;
      estimates.innerHTML = (this.currentPresupuestos || []).map(item => `<article class="glass-card p-4 rounded-2xl border border-violet-200 bg-violet-50/40 flex flex-wrap justify-between gap-3"><div><b class="text-violet-950">Presupuesto #${item.id} · ${escape(item.status)}</b><p class="text-xs mt-1">${escape(item.diagnosis)} · Vigencia: ${escape(item.validUntil || 'Sin fecha')}</p><small>${item.lines.length} procedimiento(s) · $${Number(item.total).toFixed(2)}</small></div>${item.status !== 'convertido' && window.authManager.hasPermission('invoice.write') ? `<div class="flex flex-wrap gap-2"><button class="btn-convert-estimate btn-primary px-3 py-2 rounded-xl" data-id="${item.id}">Convertir a factura</button><button class="btn-delete-estimate secondary-button px-3 py-2 rounded-xl" data-id="${item.id}">Eliminar</button></div>` : ''}</article>`).join('') || '<div class="text-xs p-3 text-slate-500">No hay presupuestos guardados para este paciente.</div>';
      ...
      estimates.querySelectorAll('.btn-delete-estimate').forEach(button => button.addEventListener('click', async () => { if (!confirm('¿Eliminar este presupuesto? Esta acción no se puede deshacer.')) return; try { await window.odontoDB.deletePresupuesto(button.dataset.id); this.currentPresupuestos = await window.odontoDB.getPresupuestos(this.currentPacienteId); this.renderFacturas(); this.showToast('Presupuesto eliminado.'); } catch (error) { this.showToast(error.message, 'error'); } }));
    }
  }
  ...
  bindEvents() {
    ...
    document.getElementById('btn-dashboard-filter')?.addEventListener('click',()=>this.loadDashboard());
    document.getElementById('btn-dashboard-export')?.addEventListener('click',()=>this.exportFinancialCSV());
    ...
    document.getElementById('btn-cash-export')?.addEventListener('click', () => this.exportCashCSV());
  ...
  formatAuditDetails(item){const changes=item.details?.changes||{};const escape=window.escapeHTML;const format=value=>{if(value===null||value===undefined||value==='')return '—';const raw=typeof value==='object'?JSON.stringify(value):String(value);return escape(raw.length>180?`${raw.slice(0,177)}…`:raw);};const rows=Object.entries(changes).slice(0,5).map(([field,value])=>`<li><b>${escape(field)}:</b> ${format(value.before)} → ${format(value.after)}</li>`).join('');return rows?`<ul class="mt-1 ml-4 list-disc text-xs text-slate-600">${rows}</ul>`:'';}
  async openDashboard(){try{const [data,audit]=await Promise.all([window.odontoDB.getDashboard(),window.odontoDB.getAudit()]);const money=value=>`RD$ ${Number(value||0).toFixed(2)}`;const escape=window.escapeHTML;document.getElementById('dashboard-content').innerHTML=`<div class="grid grid-cols-2 lg:grid-cols-4 gap-3"><article class="p-4 rounded-2xl bg-emerald-50 border"><small>Hoy</small><b class="block text-xl">${money(data.todayTotal)}</b></article><article class="p-4 rounded-2xl bg-emerald-50 border"><small>Este mes</small><b class="block text-xl">${money(data.monthTotal)}</b></article><article class="p-4 rounded-2xl bg-amber-50 border"><small>Por cobrar</small><b class="block text-xl">${money(data.outstanding)}</b></article><article class="p-4 rounded-2xl bg-blue-50 border"><small>Seguro pendiente</small><b class="block text-xl">${money(data.insurancePending)}</b></article></div><div class="grid md:grid-cols-2 gap-4 mt-5"><section class="p-4 border rounded-2xl"><h4 class="font-black">Procedimientos más realizados</h4>${data.topProcedures.map(item=>`<p class="flex justify-between border-b py-2"><span>${escape(item.name)}</span><b>${item.count}</b></p>`).join('')||'<p class="text-sm text-slate-500">Sin datos</p>'}</section><section class="p-4 border rounded-2xl"><h4 class="font-black">Producción por cajero</h4>${data.cashierProduction.map(item=>`<p class="flex justify-between border-b py-2"><span>${escape(item.name)}</span><b>${money(item.total)}</b></p>`).join('')||'<p class="text-sm text-slate-500">Sin datos</p>'}<p class="mt-3 text-sm">Anulaciones: <b>${data.voidedCount}</b> · Cierres pendientes: <b>${data.pendingApprovals}</b></p></section></div><section class="p-4 border rounded-2xl mt-4"><h4 class="font-black">Auditoría reciente</h4>${audit.slice(0,10).map(item=>`<div class="border-b py-2 text-sm"><b>${escape(item.action)}</b> · ${escape(item.userName)} <small class="text-slate-500">${new Date(item.createdAt).toLocaleString('es-DO')}</small>${this.formatAuditDetails(item)}</div>`).join('')||'<p class="text-sm text-slate-500">Sin eventos</p>'}</section>`;this.openModal('modal-dashboard');}catch(error){this.showToast(error.message,'error');}}
  async loadDashboard(){try{const from=document.getElementById('dashboard-from').value;const to=document.getElementById('dashboard-to').value;const [data,audit,report]=await Promise.all([window.odontoDB.getDashboard(),window.odontoDB.getAudit(),window.odontoDB.getFinancialReport(from,to)]);const money=value=>`RD$ ${Number(value||0).toFixed(2)}`;const escape=window.escapeHTML;const rows=(items,render)=>items.length?items.map(render).join(''):'<p class="text-sm text-slate-500">Sin datos</p>';document.getElementById('dashboard-content').innerHTML=`<div class="grid grid-cols-2 lg:grid-cols-4 gap-3"><article class="p-4 rounded-2xl bg-emerald-50 border"><small>Facturado</small><b class="block text-xl">${money(report.billed)}</b></article><article class="p-4 rounded-2xl bg-emerald-50 border"><small>Cobrado</small><b class="block text-xl">${money(report.collected)}</b></article><article class="p-4 rounded-2xl bg-blue-50 border"><small>ARS cubierto</small><b class="block text-xl">${money(report.insuranceCollected)}</b></article><article class="p-4 rounded-2xl bg-amber-50 border"><small>Por cobrar total</small><b class="block text-xl">${money(report.outstanding)}</b></article><article class="p-4 rounded-2xl bg-violet-50 border"><small>Pacientes atendidos</small><b class="block text-xl">${report.attendedPatients}</b></article><article class="p-4 rounded-2xl bg-violet-50 border"><small>Facturas cerradas</small><b class="block text-xl">${report.invoiceCount}</b></article></div><div class="grid md:grid-cols-2 gap-4 mt-5"><section class="p-4 border rounded-2xl"><h4 class="font-black">Producción facturada por odontólogo</h4>${rows(report.doctorProduction,item=>`<p class="flex justify-between border-b py-2"><span>${escape(item.name)}</span><b>${money(item.total)}</b></p>`)}</section><section class="p-4 border rounded-2xl"><h4 class="font-black">Formas de pago cobradas</h4>${rows(report.byMethod,item=>`<p class="flex justify-between border-b py-2"><span>${escape(item.method)}</span><b>${money(item.total)}</b></p>`)}</section><section class="p-4 border rounded-2xl"><h4 class="font-black">Tratamientos más realizados</h4>${rows(report.topProcedures,item=>`<p class="flex justify-between border-b py-2"><span>${escape(item.name)}</span><b>${item.count}</b></p>`)}</section><section class="p-4 border rounded-2xl"><h4 class="font-black">Resumen operativo</h4><p class="border-b py-2">Cobrado por pacientes <b class="float-right">${money(report.patientCollected)}</b></p><p class="border-b py-2">Anulaciones <b class="float-right">${data.voidedCount}</b></p><p class="py-2">Cierres pendientes <b class="float-right">${data.pendingApprovals}</b></p></section></div><section class="p-4 border rounded-2xl mt-4"><h4 class="font-black">Auditoría reciente</h4>${audit.slice(0,10).map(item=>`<div class="border-b py-2 text-sm"><b>${escape(item.action)}</b> · ${escape(item.userName)} <small class="text-slate-500">${new Date(item.createdAt).toLocaleString('es-DO')}</small>${this.formatAuditDetails(item)}</div>`).join('')||'<p class="text-sm text-slate-500">Sin eventos</p>'}</section>`;}catch(error){this.showToast(error.message,'error');}}
  async openDashboard(){const date=new Date();const today=`${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;document.getElementById('dashboard-from').value=`${today.slice(0,7)}-01`;document.getElementById('dashboard-to').value=today;this.openModal('modal-dashboard');await this.loadDashboard();}
  async loadAgenda(){try{const items=await window.odontoDB.getAppointments(document.getElementById('agenda-from').value,document.getElementById('agenda-to').value);this.agendaItems=items;const labels={pendiente:'Pendiente',confirmada:'Confirmada',atendida:'Atendida',cancelada:'Cancelada',ausente:'Ausente'};const list=document.getElementById('agenda-list');list.innerHTML=items.length?items.map(item=>`<article class="p-3 bg-white border rounded-xl"><div class="flex justify-between gap-2"><b>${window.escapeHTML(item.patientName)}</b><span class="font-black text-rose-800">${labels[item.status]}</span></div><small class="block">${new Date(item.startsAt).toLocaleString('es-DO')} – ${new Date(item.endsAt).toLocaleTimeString('es-DO',{hour:'2-digit',minute:'2-digit'})}</small><small class="block">${window.escapeHTML(item.professionalName)}</small>${item.notes?`<p class="text-xs mt-1">${window.escapeHTML(item.notes)}</p>`:''}<div class="mt-2 flex gap-2"><button class="agenda-edit secondary-button" data-id="${item.id}">Editar</button>${!['cancelada','ausente'].includes(item.status)?`<button class="agenda-cancel secondary-button" data-id="${item.id}">Cancelar</button>`:''}</div></article>`).join(''):'<p class="text-sm text-slate-500">No hay citas en este período.</p>';list.querySelectorAll('.agenda-edit').forEach(button=>button.addEventListener('click',()=>this.editAppointment(Number(button.dataset.id))));list.querySelectorAll('.agenda-cancel').forEach(button=>button.addEventListener('click',()=>this.cancelAppointment(Number(button.dataset.id))));}catch(error){this.showToast(error.message,'error');}}
  async editAppointment(id){const item=this.agendaItems?.find(row=>row.id===id);if(!item)return;const f=document.getElementById('form-appointment');f.dataset.appointmentId=String(item.id);f.patientId.value=String(item.patientId);f.professionalName.value=item.professionalName||'';f.startsAt.value=String(item.startsAt||'').slice(0,16);f.endsAt.value=String(item.endsAt||'').slice(0,16);f.status.value=item.status||'pendiente';f.notes.value=item.notes||'';f.querySelector('button[type="submit"]').textContent='Actualizar cita';f.scrollIntoView({behavior:'smooth',block:'center'});}
  async cancelAppointment(id){const item=this.agendaItems?.find(row=>row.id===id);if(!item||!confirm('¿Cancelar esta cita?'))return;try{await window.odontoDB.saveAppointment({...item,status:'cancelada'});this.showToast('Cita cancelada.');await this.loadAgenda();}catch(error){this.showToast(error.message,'error');}}
  async saveAppointment(event){event.preventDefault();const f=event.currentTarget;const appointmentId=Number(f.dataset.appointmentId||0);try{await window.odontoDB.saveAppointment({...(appointmentId?{id:appointmentId}:{}),patientId:Number(f.patientId.value),professionalName:f.professionalName.value.trim(),startsAt:f.startsAt.value,endsAt:f.endsAt.value,status:f.status.value,notes:f.notes.value.trim()});this.showToast(appointmentId?'Cita actualizada.':'Cita guardada.');delete f.dataset.appointmentId;f.startsAt.value='';f.endsAt.value='';f.notes.value='';f.querySelector('button[type="submit"]').textContent='Guardar cita';await this.loadAgenda();}catch(error){this.showToast(error.message,'error');}}
  ...
  renderCash() {
    ...
    const pendingApprovals=(report.sessions||[]).filter(item=>item.approvalStatus==='pendiente'); if(pendingApprovals.length&&this.isAdmin())document.getElementById('cash-summary').insertAdjacentHTML('beforeend',`<div class="glass-card p-4 rounded-2xl sm:col-span-3 border-amber-300"><b>Hay ${pendingApprovals.length} cierre(s) con diferencia pendiente.</b>${pendingApprovals.map(item=>`<div class="flex justify-between mt-2"><span>${escape(item.cashierName)} · ${escape(item.registerNumber)} · Diferencia ${money(item.difference)}</span><button class="cash-approve secondary-button" data-id="${item.id}">Aprobar</button></div>`).join('')}</div>`);
    document.querySelectorAll('.cash-approve').forEach(button=>button.addEventListener('click',async()=>{try{await window.odontoDB.approveCashSession(button.dataset.id);this.showToast('Cierre de caja aprobado.');await this.loadCash();}catch(error){this.showToast(error.message,'error');}}));
    ...
    pending.innerHTML = report.pendingInvoices.length ? report.pendingInvoices.map(item => `<article class="glass-card p-4 rounded-2xl border border-rose-100" data-consultation-id="${item.consultationId}">
      <div class="flex justify-between gap-3"><div><strong>${escape(item.patientName)}</strong><small class="block text-slate-500">${escape(item.date)} · ${escape(item.diagnosis)}</small><small class="block font-bold">Factura ${money(item.total)} · Pagado ${money(item.paid)}</small></div><div class="text-right"><small>Saldo pendiente</small><b class="block text-emerald-700">${money(item.remaining)}</b></div></div>
      <div class="mt-3 p-3 rounded-xl bg-blue-50 border border-blue-200"><div class="flex flex-wrap gap-3"><label class="font-bold">Cobertura del seguro (%) <input class="cash-coverage w-24 ml-2 px-3 py-2 border rounded-xl" type="number" min="0" max="100" step="0.01" value="${item.insuranceCovered ? Number(item.insuranceCovered / item.total * 100).toFixed(2) : 0}" ${item.paid || item.insuranceCovered ? 'readonly' : ''}></label><label class="font-bold">Monto a abonar <input class="cash-payment-amount w-28 ml-2 px-3 py-2 border rounded-xl" type="number" min="0.01" max="${item.remaining}" step="0.01" value="${Number(item.remaining).toFixed(2)}"></label><label class="font-bold">El paciente entregó <input class="cash-received w-28 ml-2 px-3 py-2 border rounded-xl" type="number" min="0" step="0.01" value="${Number(item.remaining).toFixed(2)}"></label></div><div class="cash-split mt-2 text-sm font-black text-blue-950">Abono: ${money(item.remaining)} · Saldo después: ${money(0)} · Devolver: ${money(0)}</div></div>
      <div class="cash-payment-lines space-y-2 mt-3">${paymentRow(Number(item.remaining).toFixed(2))}</div><button type="button" class="cash-add-method secondary-button mt-2">+ Agregar forma de pago</button>
    ...
    pending.querySelectorAll('[data-consultation-id]').forEach(card => { const calculate = () => { const invoice = report.pendingInvoices.find(i => i.consultationId === Number(card.dataset.consultationId)); const payment = Math.min(invoice.remaining, Math.max(0, Number(card.querySelector('.cash-payment-amount').value || 0))); const received = Number(card.querySelector('.cash-received').value || 0); card.querySelector('.cash-split').textContent = `Abono: ${money(payment)} · Saldo después: ${money(invoice.remaining - payment)} · Devolver: ${money(Math.max(0, received - payment))}`; }; card.querySelector('.cash-coverage').addEventListener('input', calculate); card.querySelector('.cash-payment-amount').addEventListener('input', calculate); card.querySelector('.cash-received').addEventListener('input', calculate); });
  ...
  async chargeCash(card) {
    ...
    try {
      ...
      const result = await window.odontoDB.chargeInvoice(Number(card.dataset.consultationId), { coveragePercent: Number(card.querySelector('.cash-coverage').value || 0), paymentAmount: Number(card.querySelector('.cash-payment-amount').value || 0), amountReceived: Number(card.querySelector('.cash-received').value || 0), payments, reference: card.querySelector('.cash-reference').value.trim() });
  ...
  printCashVoucher(payment, card, patientName = '', existingWindow = null) {
    ...
    receipt.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escape(payment.voucherNumber)}</title><style>
      ...
      </style></head><body><header class="brand"><img src="${logo}" alt="DentalRos"><h1>${escape(this.config.nombreClinica || 'DentalRos')}</h1><p>${escape(this.config.telefono || '')}</p></header><div class="title">COMPROBANTE DE PAGO</div><section class="info"><b>Comprobante:</b> ${escape(payment.voucherNumber)}<br><b>Factura:</b> ${escape(payment.invoiceNumber || `${String(this.config.invoicePrefix || 'FAC').toUpperCase()}-${String(payment.consultationId).padStart(8,'0')}`)}<br><b>Fecha:</b> ${new Date(payment.paidAt).toLocaleString('es-DO')}<br><b>Paciente:</b> ${escape(patient)}<br><b>Cajero:</b> ${escape(payment.receivedByName)}<br><b>Usuario:</b> @${escape(payment.cashierUsername || '')}</section>${services ? `<p class="section-title">Servicios realizados</p>${services}` : ''}<p class="section-title">Formas de pago</p>${lines}<div class="total-box"><div class="total-row main"><span>TOTAL PAGADO</span><b>${money(payment.patientPaid)}</b></div><div class="total-row"><span>Paciente entregó</span><b>${money(payment.amountReceived)}</b></div><div class="total-row change"><span>DEVOLVER</span><b>${money(payment.change)}</b></div></div><p class="thanks">¡Gracias por su pago!</p><p class="footer">${escape(this.config.piePagina || 'DentalRos · Cuidamos tu sonrisa')}</p><script>onload=()=>setTimeout(()=>print(),350)<\/script></body></html>`); receipt.document.close();
  }

  downloadCSV(filename, headers, rows){const quote=value=>`"${String(value??'').replace(/"/g,'""')}"`;const content=`\uFEFF${headers.map(quote).join(',')}\n${rows.map(row=>row.map(quote).join(',')).join('\n')}`;const blob=new Blob([content],{type:'text/csv;charset=utf-8'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=filename;link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000);}
  exportCashCSV(){if(!this.cashReport)return this.showToast('Carga primero el reporte de caja.','error');const report=this.cashReport;this.downloadCSV(`Caja_${report.from}_${report.to}.csv`,['Comprobante','Factura','Fecha','Paciente','Cajero','Métodos','Cobrado paciente','ARS','Entregado','Devuelta','Estado'],report.payments.map(item=>[item.voucherNumber,item.invoiceNumber,new Date(item.paidAt).toLocaleString('es-DO'),item.patientName,item.receivedByName,(item.lines||[]).map(line=>`${line.method}: ${line.amount}`).join(' | '),item.patientPaid,item.insuranceCovered,item.amountReceived,item.change,item.status]));this.downloadCSV(`Cuentas_por_cobrar_${report.from}_${report.to}.csv`,['Factura','Paciente','Fecha','Diagnóstico','Total','Pagado','ARS','Saldo'],report.pendingInvoices.map(item=>[item.consultationId,item.patientName,item.date,item.diagnosis,item.total,item.paid,item.insuranceCovered,item.remaining]));this.showToast('Se descargaron Caja y Cuentas por cobrar en CSV.');}
  async exportFinancialCSV(){try{const from=document.getElementById('dashboard-from').value;const to=document.getElementById('dashboard-to').value;const report=await window.odontoDB.getFinancialReport(from,to);const rows=[['Resumen','Facturado',report.billed],['Resumen','Cobrado',report.collected],['Resumen','Cobrado por pacientes',report.patientCollected],['Resumen','Cubierto por ARS',report.insuranceCollected],['Resumen','Cuentas por cobrar',report.outstanding],...report.byMethod.map(item=>['Método de pago',item.method,item.total]),...report.doctorProduction.map(item=>['Producción por odontólogo',item.name,item.total]),...report.topProcedures.map(item=>['Tratamiento',item.name,item.count])];this.downloadCSV(`Reporte_financiero_${from}_${to}.csv`,['Categoría','Concepto','Valor'],rows);this.showToast('Reporte financiero CSV descargado.');}catch(error){this.showToast(error.message,'error');}}
  printCashReport() {
