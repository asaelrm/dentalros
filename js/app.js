...
class OdontoApp {
  ...
  renderCash() {
    ...
    pending.innerHTML = report.pendingInvoices.length ? report.pendingInvoices.map(item => `<article class="glass-card p-4 rounded-2xl border border-rose-100" data-consultation-id="${item.consultationId}">
      ...
      <div class="mt-3 p-3 rounded-xl bg-blue-50 border border-blue-200"><label class="font-bold">Cobertura del seguro (%) <input class="cash-coverage w-24 ml-2 px-3 py-2 border rounded-xl" type="number" min="0" max="100" step="0.01" value="0"></label><div class="cash-split mt-2 text-xs font-bold">Seguro: ${money(0)} · Paga paciente: ${money(item.total)}</div></div>
      <div class="flex flex-wrap gap-2 mt-3"><select class="cash-method px-3 py-2 border rounded-xl"><option value="efectivo">Efectivo</option><option value="tarjeta">Tarjeta</option><option value="transferencia">Transferencia</option><option value="otro">Otro</option></select><input class="cash-reference flex-1 min-w-32 px-3 py-2 border rounded-xl" maxlength="150" placeholder="Referencia (opcional)"><button class="cash-charge btn-primary px-4 py-2 rounded-xl font-black">Registrar cobro</button></div>
    ...
    pending.querySelectorAll('.cash-coverage').forEach(input => input.addEventListener('input', () => { const card = input.closest('[data-consultation-id]'); const invoice = report.pendingInvoices.find(i => i.consultationId === Number(card.dataset.consultationId)); const covered = invoice.total * Math.min(100, Math.max(0, Number(input.value || 0))) / 100; card.querySelector('.cash-split').textContent = `Seguro: ${money(covered)} · Paga paciente: ${money(invoice.total - covered)}`; }));
    document.getElementById('cash-payments').innerHTML = report.payments.length ? report.payments.map(item => `<article class="glass-card p-4 rounded-2xl flex justify-between gap-3"><div><strong>${escape(item.patientName)}</strong><small class="block text-slate-500">${new Date(item.paidAt).toLocaleString('es-ES')} · ${methodLabels[item.paymentMethod] || escape(item.paymentMethod)}</small><small class="block">Seguro ${item.coveragePercent}%: ${money(item.insuranceCovered)} · Paciente: ${money(item.patientPaid)}</small><small class="block">Cobró: ${escape(item.receivedByName)}${item.reference ? ` · Ref: ${escape(item.reference)}` : ''}</small></div><b class="text-emerald-700">${money(item.patientPaid)}</b></article>`).join('') : '<p class="text-sm text-slate-500 p-4">No hay cobros en este período.</p>';
  }

  async chargeCash(card) {
    const button = card.querySelector('.cash-charge'); button.disabled = true;
    try {
      await window.odontoDB.chargeInvoice(Number(card.dataset.consultationId), { coveragePercent: Number(card.querySelector('.cash-coverage').value || 0), paymentMethod: card.querySelector('.cash-method').value, reference: card.querySelector('.cash-reference').value.trim() });
