...
class OdontoApp {
  ...
  async editAppointment(id){const item=this.agendaItems?.find(row=>row.id===id);if(!item)return;const notes=prompt('Observaciones de la cita:',item.notes||'');if(notes===null)return;try{await window.odontoDB.saveAppointment({...item,notes});this.showToast('Cita actualizada.');await this.loadAgenda();}catch(error){this.showToast(error.message,'error');}}
