...
function isKnownProtectedPath(pathname) {
  return pathname === '/api/auth/me' ||
    ...
    pathname === '/api/users' ||
    pathname === '/api/insurers' || pathname === '/api/inventory' || /^\/api\/inventory\/\d+(?:\/movements)?$/.test(pathname) || pathname === '/api/backups' || pathname === '/api/dashboard' || pathname === '/api/reports/financial' || pathname === '/api/audit' || pathname === '/api/appointments' || /^\/api\/appointments\/\d+$/.test(pathname) ||
    ...
    /^\/api\/pacientes\/\d+(?:\/(?:historia|consultas|odontograma|odontograma\/historial|adjuntos|firmas|consentimientos|presupuestos))?$/.test(pathname) ||
...
async function handleApi(req, res, pathname, context) {
  ...
  const inventoryMatch=pathname.match(/^\/api\/inventory\/(\d+)(?:\/movements)?$/);
  if(pathname==='/api/inventory'&&method==='GET'){requirePermission(auth,'catalog.write');return sendJson(res,200,db.prepare('SELECT * FROM inventory_items ORDER BY name').all().map(row=>({id:Number(row.id),name:row.name,sku:row.sku,unit:row.unit,stock:Number(row.stock),minimumStock:Number(row.minimum_stock),supplier:row.supplier,expiryDate:row.expiry_date,active:Boolean(row.active)})));}
  if(pathname==='/api/inventory'&&method==='POST'){requirePermission(auth,'catalog.write');const body=requireObject(await readJsonBody(req,bodyLimit));const name=String(body.name||'').trim();const sku=String(body.sku||'').trim();const stock=Number(body.stock||0),minimumStock=Number(body.minimumStock||0);if(!name||name.length>150||sku.length>80||!Number.isFinite(stock)||stock<0||!Number.isFinite(minimumStock)||minimumStock<0)throw new HttpError(400,'Los datos del material no son válidos.');const now=nowIso();const id=Number(db.prepare('INSERT INTO inventory_items(name,sku,unit,stock,minimum_stock,supplier,expiry_date,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)').run(name,sku||`MAT-${Date.now()}`,String(body.unit||'unidad').slice(0,40),stock,minimumStock,String(body.supplier||'').slice(0,150),String(body.expiryDate||''),now,now).lastInsertRowid);writeAudit(db,auth.user.id,'inventory_item_create','inventory_item',id,{name,stock,minimumStock});return sendJson(res,201,{id,name,sku:sku||`MAT-${Date.now()}`,stock,minimumStock});}
  if(inventoryMatch&&pathname.endsWith('/movements')&&method==='POST'){requirePermission(auth,'catalog.write');const itemId=positiveId(inventoryMatch[1],'Material');const body=requireObject(await readJsonBody(req,bodyLimit));const type=String(body.type||'');const quantity=Number(body.quantity);if(!['entrada','salida','ajuste'].includes(type)||!Number.isFinite(quantity)||quantity<=0)throw new HttpError(400,'El movimiento de inventario no es válido.');const item=db.prepare('SELECT stock FROM inventory_items WHERE id=?').get(itemId);if(!item)throw new HttpError(404,'Material no encontrado.');const next=type==='entrada'?Number(item.stock)+quantity:type==='salida'?Number(item.stock)-quantity:quantity;if(next<0)throw new HttpError(409,'No hay existencias suficientes.');const now=nowIso();runTransaction(db,()=>{db.prepare('UPDATE inventory_items SET stock=?,updated_at=? WHERE id=?').run(next,now,itemId);db.prepare('INSERT INTO inventory_movements(item_id,movement_type,quantity,notes,created_by,created_by_name,created_at) VALUES(?,?,?,?,?,?,?)').run(itemId,type,quantity,String(body.notes||'').slice(0,300),auth.user.id,auth.user.displayName,now);writeAudit(db,auth.user.id,'inventory_movement','inventory_item',itemId,{type,quantity,before:Number(item.stock),after:next});});return sendJson(res,201,{itemId,stock:next});}
  ...
  const patientConsentsMatch = pathname.match(/^\/api\/pacientes\/(\d+)\/consentimientos$/);
  if (patientConsentsMatch && method === 'GET') { const patientId=positiveId(patientConsentsMatch[1],'ID de paciente'); ensurePatient(db,patientId); const rows=db.prepare('SELECT * FROM informed_consents WHERE patient_id=? ORDER BY id DESC').all(patientId); return sendJson(res,200,rows.map(row=>({id:Number(row.id),patientId:Number(row.patient_id),procedureName:row.procedure_name,content:row.content,patientSignatureId:row.patient_signature_id==null?null:Number(row.patient_signature_id),professionalSignatureId:row.professional_signature_id==null?null:Number(row.professional_signature_id),createdByName:row.created_by_name,createdAt:row.created_at}))); }
  if (patientConsentsMatch && method === 'POST') { requireRole(auth,CLINICAL_WRITERS); const patientId=positiveId(patientConsentsMatch[1],'ID de paciente'); ensurePatient(db,patientId); const body=requireObject(await readJsonBody(req,bodyLimit)); const procedureName=String(body.procedureName||'').trim(); const content=String(body.content||'').trim(); if(!procedureName||procedureName.length>200||!content||content.length>12000) throw new HttpError(400,'El procedimiento y el texto del consentimiento son obligatorios.'); const signatureId=value=>value==null||value===''?null:positiveId(value,'Firma'); const patientSignatureId=signatureId(body.patientSignatureId); const professionalSignatureId=signatureId(body.professionalSignatureId); for(const id of [patientSignatureId,professionalSignatureId])if(id&&!db.prepare('SELECT 1 FROM clinical_signatures WHERE id=? AND patient_id=?').get(id,patientId))throw new HttpError(400,'La firma no pertenece a este paciente.'); const createdAt=nowIso(); const id=Number(db.prepare('INSERT INTO informed_consents(patient_id,procedure_name,content,patient_signature_id,professional_signature_id,created_by,created_by_name,created_at) VALUES(?,?,?,?,?,?,?,?)').run(patientId,procedureName,content,patientSignatureId,professionalSignatureId,auth.user.id,auth.user.displayName,createdAt).lastInsertRowid); writeAudit(db,auth.user.id,'informed_consent_create','informed_consent',id,{patientId,procedureName,patientSignatureId,professionalSignatureId}); return sendJson(res,201,{id,patientId,procedureName,content,patientSignatureId,professionalSignatureId,createdByName:auth.user.displayName,createdAt}); }
