...
async function handleApi(req, res, pathname, context) {
  ...
  if (pathname === '/api/cash' && method === 'GET') {
    ...
    let payments = db.prepare(`SELECT cp.*, p.data AS patient_data, json_extract(p.data, '$.nombre') AS patient_name,
      ...
      })).map(payment => { const consultation = parseData(db.prepare('SELECT data FROM consultas WHERE id = ?').get(payment.consultationId)?.data || '{}'); return ({ ...payment, consultation: { ...consultation, id: payment.consultationId, pacienteId: payment.patientId }, cashierUsername: payment.receivedBy ? (db.prepare('SELECT username FROM users WHERE id = ?').get(payment.receivedBy)?.username || '') : '', voucherNumber: `REC-${String(payment.id).padStart(8, '0')}`, invoiceNumber: `FAC-${String(payment.consultationId).padStart(8, '0')}`, services: consultation.factura?.procedimientos || [], lines: db.prepare('SELECT method, amount_centavos, card_brand, card_type, last_four, authorization_number, reference_number, processor FROM cash_payment_lines WHERE cash_payment_id = ? ORDER BY id').all(payment.id).map(line => ({ method: line.method, amount: Number(line.amount_centavos) / 100, cardBrand: line.card_brand, cardType: line.card_type, lastFour: line.last_four, authorizationNumber: line.authorization_number, referenceNumber: line.reference_number, processor: line.processor })) }); });
